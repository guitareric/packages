const createPage = require('../utils/puppeteer')
const { parseBotType, handlePage } = require('./../utils/zillow-helpers')

;(async () => {
  const botType = parseBotType(process.argv[1])
  const url =
  ...
  let finalResults = await handlePage(page, browser)
  // finalResults = finalResults.replace(/}/g, '}').replace(/{/g, '{')
